...
.pull-left[
```{r humphrey_2001, echo=FALSE, fig.align='center', out.height= 350, out.width=700}
...
.pull-right[
```{r alexander_2006, echo=FALSE, fig.align='center', out.height=500, out.width=275}
...
* Analysing species interactions may allow us to predict the species which can be supported in 
...
## Associated Species - Sample (redo)
...
## Differences Between Tree Groups

```{r species_time_box, echo=FALSE, warning=FALSE, fig.align='center', include=TRUE, fig.width=18, fig.height=9, dpi=600}

data <- read.csv("Output_Data/species_time_data.csv") |> 
 dplyr::mutate_if(is.numeric, tidyr::replace_na, 0)


mult <- c(1, 1.03, 1.06, 1.08, 1.1, 1.14)
...
```{r species_time, echo=FALSE, warning=FALSE, cache=FALSE, message=FALSE, out.width="100%", fig.align='center'}

data <- read.csv("Output_Data/species_time_data.csv") #|> 
  # dplyr::mutate_if(is.numeric, tidyr::replace_na, 0)

# Models for data grouped by tree group
modelsG <- data |> 
  dplyr::group_by(Taxon_group, Tree_group) |> 
  dplyr::summarise(model = list(stats::lm(log10(Associated_Species) ~ log10(Growing_Time))))

modelsG_summary <- data.frame(Taxon_group = modelsG$Taxon_group, Tree_group = modelsG$Tree_group) |> 
  cbind(purrr::map_df(modelsG$model, broom::glance))

# Models for data ungrouped by tree group
models <- data |> 
  dplyr::group_by(Taxon_group) |> 
  dplyr::summarise(model = list(stats::lm(log10(Associated_Species) ~ log10(Growing_Time))))

models_summary <- data.frame(Taxon_group = models$Taxon_group) |> 
  cbind(purrr::map_df(models$model, broom::glance))

# r2_annot_total <- data.frame(
#   label = c(paste("R2: ", round(models_summary$r.squared, 2))),
#   Taxon_group = c(models_summary$Taxon_group)
# )
...
DT::datatable(
  ...
  DT::formatStyle(columns = 1:ncol(modelsG_summary), fontSize = '50%')
...
### Rhopalosiphum nymphaeae eats Lemma gibba
