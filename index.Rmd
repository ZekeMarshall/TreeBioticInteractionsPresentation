...
```{r network, echo=FALSE, message=FALSE, warning=FALSE, dpi=1000}

data <- read.csv("Output_Data/a1_assoc_spp.csv") |> 
  dplyr::slice(-1)

nvc_spp <- unique(data$NVC.species)

assoc_spp <- unique(data$Associated.species)
...
edges <- data |> 
  dplyr::select(NVC.species, Associated.species) |> 
...
foo <- visNetwork::visNetwork(nodes, edges) |> 
  ...
  visNetwork::visGroups(groupname = "eats", color = "#f75b40", alpha = 0.5) |> 
